export default ({ config, log }) => ({
  ...
  error: {
    ...
    showLocations: !config.env.prod, // whether to add the graphql locations to the final error (default false)
    showPath: !config.env.prod, // whether to add the graphql path to the final error (default false)
    hideSensitiveData: config.env.prod, // whether to remove the data object from internal server errors (default true)
    hooks: {
